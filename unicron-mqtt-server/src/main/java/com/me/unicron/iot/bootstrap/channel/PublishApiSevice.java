...
import com.me.unicron.iot.util.MessageId;
...
//        dup保证消息可靠传输，默认为0，只占用一个字节，表示第一次发送。不能用于检测消息重复发送等
    ...
    private SendMqttMessage  addQos2Queue(Channel channel,int messageId,String topic,byte[] datas,MqttQoS mqttQoS,ConfirmStatus confirmStatus){
        ...
        //限流降级处理
        if(!scanRunnable.isExceedMax()){
        	//FileUtils.appendFile(que_alarm_log, "缓存消息队列大小告警："+DateUtils.getCurLongDateStr()+","+scanRunnable.getQueueSize()+"\r\n");
        	//scanRunnable.addQos2Queue(build);
        	scanRunnable.addQueue(build);
        }
        return build;
    }
