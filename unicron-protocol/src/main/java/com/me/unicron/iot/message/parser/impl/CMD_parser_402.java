...
@Slf4j
public class CMD_parser_402 implements BaseParser {

    public HistoryOrderInfoResponse_402 parse(String headStartStr,byte[] bytes) {
    	final String NULL_RESULT = "0";
    	final String SUCCESS_RESULT = "0000";
    	
        byte[] shortByte = new byte[2];
        byte[] intByte = new byte[4];
        byte[] longByte = new byte[8];
        byte[] byte32 = new byte[32];
        HistoryOrderInfoResponse_402 chargeInfo = new HistoryOrderInfoResponse_402();
        try {
            for (int i = 0; i < bytes.length;) {
                ...
                chargeInfo.setIndex(String.valueOf(EncodeUtil.byteToInt(intByte)));
                i += 4;
                //总充电记录条目
                System.arraycopy(bytes, i, intByte, 0, 4);
                chargeInfo.setRecord(String.valueOf(EncodeUtil.byteToInt(intByte)));
                i += 4;
                //充电枪类型
                byte portType = bytes[i];
                ...
                chargeInfo.setStartTime(String.valueOf(EncodeUtil.byteDateToDateStr(longByte)));
                i += 8;
                //充电结束时间
                System.arraycopy(bytes, i, longByte, 0, 8);
                chargeInfo.setEndTime(String.valueOf(EncodeUtil.byteDateToDateStr(longByte)));
                i += 8;
                //充电时间长度
                System.arraycopy(bytes, i, intByte, 0, 4);
                ...
                for (int j = 1; j <= 48; j++) {
                    ...
                    Method powerPeriod;
                    try {
                        powerPeriod = CMD_parser_202.class.getMethod("setPowerPeriod" + i, String.class);
                        powerPeriod.invoke(CMD_parser_202.class, powerPeriodStr);
                    } catch (Exception e) {
